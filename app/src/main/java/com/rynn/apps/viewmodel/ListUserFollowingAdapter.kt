...
class ListUserFollowingAdapter(private val listUserFollowing: ArrayList<UserFollowing>) :
    RecyclerView.Adapter<ListUserFollowingAdapter.ListUserHolder>() {
    ...
    inner class ListUserHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(userFollowing: UserFollowing) {
            with(itemView) {
                ...
                user_repo.text =
                    StringBuilder(resources.getString(R.string.user_repo)).append(": ${userFollowing.repository}")
                user_followers.text =
                    StringBuilder(resources.getString(R.string.follower)).append(": ${userFollowing.followers}")
                user_following.text =
                    StringBuilder(resources.getString(R.string.following)).append(": ${userFollowing.following}")
            }
        }
    }
}